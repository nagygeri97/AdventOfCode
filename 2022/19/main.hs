...
nextStates (oreOre, clayOre, (obsiOre, obsiClay), (geoOre, geoObsi)) (time, qty@(oreQ, clayQ, obsiQ, geoQ), bot@(oreB, clayB, obsiB, geoB)) 
    | not (null statesAfterGeoBuy) = statesAfterGeoBuy
    | not (null statesAfterObsiBuy) = statesAfterObsiBuy
    | otherwise = statesAfterNoBuy ++ statesAfterOreBuy ++ statesAfterClayBuy
    where
...
iterateStates n costs time states 
    ...
    where 
        newStates = Set.toList . Set.fromList $ (concat . map (nextStates costs) $ states)
...
solve2 lines = map (\cost -> getMaxGeo $ iterateStates 32 cost 0 [initialState]) . take 3 $ lines 