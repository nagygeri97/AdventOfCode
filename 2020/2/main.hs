...
main = length . filter isValid $ input
main2 = length . filter isValid2 $ input

isValid (mi, ma, c, s) = cnt <= ma && cnt >= mi
    where 
        cnt = len cs
        len [] = 0
        len [x] = length x  
        cs = filter ((==c) . head) . group . sort $ s

isValid2 (a,b,c,s) = (s !! (a - 1) == c && s !! (b - 1) /= c) || (s !! (a - 1) /= c && s !! (b - 1) == c)
